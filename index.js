...
async function restoreRoles(member) {
    ...
    if (!botMember) {
        console.error("❌ Không lấy được thông tin role của bot.");
        return;
    }
...
client.on(
    ...
    async (oldMember, newMember) => {
        ...
        const oldJail = oldMember.roles.cache.has(
            JAIL_ROLE_ID
        );

        const newJail = newMember.roles.cache.has(
            JAIL_ROLE_ID
        );
